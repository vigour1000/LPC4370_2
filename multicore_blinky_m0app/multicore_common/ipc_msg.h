 ...
 */
extern int (*IPC_EventHandler)(IPC_EVENT_T event, uint16_t cpu, int tout);
 ...
 */
static INLINE int IPC_tryPushMsg(uint16_t cpuid, const void *data)
{
	return IPC_pushMsgTout(cpuid, data, 0);
}
 ...
 */
static INLINE int IPC_pushMsg(uint16_t cpuid, const void *data)
{
	return IPC_pushMsgTout(cpuid, data, -1);
}
